...
tmux new-window -t $SESSION:10 -n 'Behavior Coordinator'
...
tmux new-window -t $SESSION:11 -n 'Hector Slam'
...
tmux new-window -t $SESSION:12 -n 'Move Base'
...
tmux new-window -t $SESSION:13 -n 'Navigation with lidar'
tmux send-keys "roslaunch navigation_with_lidar navigation_with_lidar.launch --wait \
  namespace:=drone$NUMID_DRONE \
  clearance:=0.33" C-m

tmux new-window -t $SESSION:14 -n 'rviz'
